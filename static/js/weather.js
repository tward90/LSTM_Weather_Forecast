...
d3.json("dummy_data.json").then((weatherData) => {
  ...
  .enter()
  .append('p')
  .html(d => `<div> <strong>Temperature</strong>: ${d.temp.toFixed(0)}° <br> <strong>Low Temp</strong>: ${d.min.toFixed(0)}° <br> <strong>High Temp</strong>: ${d.max.toFixed(0)}° <br><strong>Chance of Rain</strong>: ${d.rain_drizzle * 100}%</div>`)
